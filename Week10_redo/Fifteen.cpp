...
// New class for counting words containing 'z'
class WordsWithZCounter {
public:
  WordsWithZCounter(WordFrequencyFramework &wfapp, DataStorage &data_storage) {
    data_storage.word_event.connect(
        [this](const string &word) { check_word(word); });
    wfapp.end_event.connect([this]() { print_count(); });
  }

private:
  int z_word_count = 0;

  void check_word(const string &word) {
    if (word.find('z') != string::npos) {
      z_word_count++;
    }
  }

  void print_count() { cout << "Words with 'z': " << z_word_count << endl; }
};
...
int main(int argc, char *argv[]) {
  ...
  WordsWithZCounter words_with_z_counter(wfapp, data_storage); //15.2
