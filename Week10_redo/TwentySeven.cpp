...
public:
    ...
    void updateCleanedTokens(allWordsColumn tokenObj, stopWordsColumn stopObj){
        cleanedTokens.clear();  // Recompute cleaned tokens from cumulative data
...
public:
    vector<string> uniqueWords;
    void updateUniqueWords(const cleanedTokensColumn& cleanedObj) {
        set<string> temp(cleanedObj.cleanedTokens.begin(), cleanedObj.cleanedTokens.end());
        uniqueWords.clear();  // Recompute unique words from cumulative cleaned tokens
        uniqueWords.insert(uniqueWords.end(), temp.begin(), temp.end());
    }
};
...
public:
    ...
    void updateWordFreq(cleanedTokensColumn cleanedObj, uniqueWordsColumn uniqueObj){
        ...
        counts.clear();  // Recompute counts from cumulative cleaned tokens
...
class sortFreqFunction{
    ...
    public:
        ...
        void sortFreq(uniqueWordsColumn uniqueObj, countsColumn countObj){ 
            ...
            sortedFreq.clear();  // Recompute sorted frequencies
...
int main(int argc, char *argv[]){
    ...
    while(true){
        // ask for new input, add to the word space and print top 25 freq
        string newFile;
        cout << "Enter new file name: ";
        cin >> newFile;
        sheet.updateSpreadsheet(newFile, "../stop_words.txt");
        sheet.sort.printTopFreq();
